...
def detect_prob(disc_vol_scaled):
  ...
  if ratio < 0.002: #set very low ratios to probability of 0
    ...
    detection_prob = 1
  else: detection_prob  = -0.004757 * ratio**4 + 0.06681 * ratio**3 - 0.3605 * ratio**2 + 0.9215 * ratio + 0.00826
  return round(detection_prob, 3)

def random_polar_coordinate(disc_radius_scaled):
...
def star_haze(disc_radius_scaled, density):
  """randomly disribute faint tkinter stars in galactic disc.
  
  disc_radius_scaled = galactic disc radius scaled to radio bubble diamter
  density = multiplier to vary number of stars posted
  """
  
  for i in range(0, disc_radius_scaled * density):
    x, y = random_polar_coordinate(disc_radius_scaled)
