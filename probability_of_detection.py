...
for nums_civs in range(2, MAX_CIVS + 2, CIV_STEP_SIZE):
  civs_per_vol = nums_civs / NUM_EQUIV_VOLUMES
  num_single_civs = 0
  for trial in range(TRIALS):
    locations = [] #equivalent volumes containing a civilization
    while len(locations) < nums_civs:
  ...
  prob = 1 - (num_single_civs / (nums_civs * TRIALS))
